...
public interface BookingService extends BookingServiceSort {
    ...
    List<BookingDTO.Controller.ReturnBookItemDTO> findByBooker(Long bookerId, BookingStateForOutput state, Integer from, Integer size);

    List<BookingDTO.Controller.ReturnBookItemDTO> findByOwner(Long bookerId, BookingStateForOutput state, Integer from, Integer size);
