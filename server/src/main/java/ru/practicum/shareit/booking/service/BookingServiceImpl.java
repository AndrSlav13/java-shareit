...
public class BookingServiceImpl implements BookingService {
    ...
    @Override
    public List<BookingDTO.Controller.ReturnBookItemDTO> findByBooker(Long bookerId, BookingStateForOutput state, Integer from, Integer size) {
        User booker = userService.getSimpleUser(bookerId);
        UserService.validate(booker);

        return bookingStore.findByBooker(bookerId, state, from, size).stream()
                .map(booking -> BookingDTO.Controller.Mapper.toReturnBookItemDTO(booking))
                .collect(Collectors.toList());
    }

    @Override
    public List<BookingDTO.Controller.ReturnBookItemDTO> findByOwner(Long bookerId, BookingStateForOutput state, Integer from, Integer size) {
        User booker = userService.getSimpleUser(bookerId);
        UserService.validate(booker);

        return bookingStore.findByOwner(bookerId, state, from, size).stream()
                .map(booking -> BookingDTO.Controller.Mapper.toReturnBookItemDTO(booking))
                .collect(Collectors.toList());
    }
}
